...
def jacobi(a, b, eps=1e-5):
    ...
    for itr in range(MAX_ITER_COUNT):
        ...
        # print('Before: ', proc_id, x)

        x_recv = np.empty(x.shape)
        if proc_id == 0:
            # First process in the chain
            comm.Send([x, MPI.FLOAT], dest=proc_id + 1)
            # print(f'First sent to {proc_id + 1}')
            comm.Recv([x_recv, MPI.FLOAT], source=proc_id + 1)
            # print(f'First received from {proc_id + 1}')
            x_recv[proc_slice] = x[proc_slice]
            x = x_recv
        elif proc_id == proc_count - 1:
            # Last process in the chain
            comm.Recv([x_recv, MPI.FLOAT], source=proc_id - 1)
            # print('Last process received: ', x_recv)
            x[:proc_slice.start] = x_recv[:proc_slice.start]
            # print(f'Last received from {proc_id - 1}')
            comm.Send([x, MPI.FLOAT], dest=proc_id - 1)
            # print(f'Last sent to {proc_id - 1}')
        else:
            # Some middle process
            comm.Recv([x_recv, MPI.FLOAT], source=proc_id - 1)
            # print(f'#{proc_id} received from {proc_id - 1}')
            x_recv[proc_slice] = x[proc_slice]
            x = x_recv
            comm.Send([x, MPI.FLOAT], dest=proc_id + 1)
            # print(f'#{proc_id} sent to {proc_id + 1}')
            comm.Recv([x_recv, MPI.FLOAT], source=proc_id + 1)
            # print(f'#{proc_id} received from {proc_id + 1}')
        ...
        # print('After: ', proc_id, x)

        # if iters == 2:
        #     print(proc_id, x)

        if np.linalg.norm(x - x_prev, np.inf) < eps:
            # print(f'#{proc_id} exiting after {iters}')
            break
    else:
...
if proc_id == 0:
    # Save results
    with open('jacobi.csv', 'a') as fp:
        writer = csv.writer(fp)
        writer.writerow([N, proc_count, np.mean(times)])
